...
def get_db_by_index (conn_str, db_start_index, db_end_index):

   LIST_db_ALL = get_ALL_db_names (conn_str)
   
   LIST_db_selected = LIST_db_ALL[db_start_index, db_end_index]
   
   
   
def get_db_by_name (conn_str, my_db_name):

   LIST_db_ALL = get_ALL_db_names (conn_str)
   
   LIST_db_selected = []
   
   for db_name in LIST_db_ALL:
   
      if my_db_name not in db_name:
      
         print (my_db_name + ' NOT in ' + db_name)
         
      else:
      
         LIST_db_selected.append(db_name)
...
def get_collection_by_index (conn_str, db_name, coll_start_index, coll_end_index):

   LIST_coll_ALL = get_ALL_collection_in_db (conn_str, db_name)
   
   LIST_coll_selected = LIST_coll_ALL [coll_start_index: coll_end_index]
   
   return LIST_coll_selected 
   
   
   
def get_collection_by_name (conn_str, db_name, my_coll_name):

   LIST_coll_ALL = get_ALL_collection_in_db (conn_str, db_name)
   
   LIST_coll_selected = []

   for coll_name in LIST_coll_ALL:
   
      if my_coll_name not in coll_name:
      
         print (my_coll_name + ' NOT in ' + coll_name)
         
      else:
      
         LIST_coll_selected.append (coll_name)
...
'''            
returned_id = create_ONE_document (conn_str ="mongodb://localhost:27017/", db_name = "mydatabase",  coll_name="mycoll", doc_data = doc_data)
...
'''
#print (LIST_all_doc[0])

### a doc is already a dict, no need to convert into a dict
print (' data type of doc => ' + str(type(LIST_all_doc[0])))

LIST_doc_item = LIST_all_doc[0].items()

LIST_doc_value = LIST_all_doc[0].values()


for doc_key_value in LIST_doc_item:

   print (doc_key_value)


for doc_value in LIST_doc_value:
...
for doc in LIST_all_doc:
   ...
   doc_value_1 = doc[doc_key_1]
   doc_value_2 = doc[doc_key_2]

   print(doc_value_1 + ' => ' + doc_value_2) # get values by key
'''
...
for doc in LIST_all_doc:
   ...
   if 'Amy' in doc_str or 'John' in doc_str:
...
def get_doc_by_index (conn_str, db_name, coll_name, doc_start_index, doc_end_index):

   LIST_doc_ALL = get_ALL_docs_in_collection (conn_str, db_name, coll_name)
   
   LIST_doc_selected = LIST_doc_ALL [doc_start_index: doc_end_index]
...
def get_doc_by_name (conn_str, db_name, coll_name, my_doc_name):

   LIST_doc_ALL =get_ALL_docs_in_collection (conn_str, db_name, coll_name)
   
   LIST_doc_selected = []

   for doc_name in LIST_coll_ALL:
   
      if my_doc_name not in doc_name:
      
         print (my_doc_name + ' NOT in ' + doc_name)
         
      else:
      
         LIST_doc_selected.append (doc_name)
...
'''
# Update doc by returned id 
returned_id = create_ONE_document (conn_str ="mongodb://localhost:27017/", db_name = "mydatabase",  coll_name="mycoll", doc_data = doc_data)

print ("hello man => " + str(datetime.datetime.now()), flush=True)
time.sleep(5)
print ("yes Shane => " + str(datetime.datetime.now()), flush=True)

update_ONE_document (conn_str ="mongodb://localhost:27017/", db_name = "mydatabase",  coll_name="mycoll", values_OLD={ "_id": ObjectId (returned_id) }, values_NEW={ "$set": { "name": "changed name" } })
'''



def update_MANY_document (conn_str):
