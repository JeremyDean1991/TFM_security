...
def get_page_source (path_Chromedriver, url, random_proxy):

   #LIST_proxy_ip_port = get_proxy_list()
   #random_proxy = random_select_from_list (LIST_item=LIST_proxy_ip_port)

   d_Chrome = use_Chromedriver (path_Chromedriver, url, headless_enabled='no', js_enabled='yes',user_agent_enabled ='no', proxy_ip_port=random_proxy)   
...
#get_page_source (path_Chromedriver="../../data/raw/Chromedrivers/chromedriver-win64/chromedriver.exe", url='https://duckduckgo.com/?t=h_&q=python&ia=web', random_proxy='')  

 

def get_page_title (path_Chromedriver, url, random_proxy):

   d_Chrome = use_Chromedriver (path_Chromedriver, url, headless_enabled='no', js_enabled='yes',user_agent_enabled ='no', proxy_ip_port=random_proxy) 
 
   page_title = d_Chrome.title
   
   #print (page_title, flush=True)
   return page_title 
 

''' 
urlX =  'https://www.msn.com/en-au/news/other/elon-musk-wants-to-abolish-daylight-savings-time-and-it-might-actually-be-a-good-idea/ar-AA1vhPvk?ocid=hpmsn&cvid=49f56cc650874a2f883cb02580c89c35&ei=17'
get_page_title (path_Chromedriver="../../data/raw/Chromedrivers/chromedriver-win64/chromedriver.exe", url=urlX, random_proxy='')     
''' 
...
def get_ALL_links_by_xpath (path_Chromedriver, page_url, random_proxy):

   LIST_page_link = []
   
   # to get all page links, must set js_enabled = 'yes'
   d_Chrome = use_Chromedriver (path_Chromedriver=path_Chromedriver, url=page_url, headless_enabled='yes', js_enabled='yes',user_agent_enabled ='no', proxy_ip_port=random_proxy)
   
   elems = d_Chrome.find_elements(By.XPATH, "//a[@href]")
   ...
   for elem in elems:
      ...
      try:
         ...
         #print(page_link, flush=True)
      
         LIST_page_link.append(page_link)
         
      except:
         
         print ('an execption has occured', flush=True)
      
   return LIST_page_link
 
'''
url_1 = 'https://www.youtube.com/watch?v=d5nNQt9NiII'  
Chrome_path = "../../data/raw/Chromedrivers/chromedriver-win64/chromedriver.exe"

get_ALL_links_by_xpath (path_Chromedriver =Chrome_path,  page_url=url_1, random_proxy='')   
'''  
